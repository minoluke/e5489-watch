...
| `partySize` | 何名で乗るか。**この人数を確実に確保できる場合だけ通知します**（下記） |
...
| --- | --- | --- |
| ○ | 空席あり | 11席以上 |
| △ | 空席残りわずか | 1席（1〜10席のため**1席しかない可能性がある**） |
| × | 満席 | 0 |
...
この2つを突き合わせ、**`partySize` 名を確実に確保できるときだけ通知**します。
`partySize: 2` の場合:

| 設備 | ○ | △ | × |
| --- | --- | --- | --- |
| サンライズツイン・シングルツイン | 通知 | **通知** | 見送り |
| それ以外（1人用） | 通知 | **見送り** | 見送り |

1人用設備の △ を見送るのは、1席しかなければ2名で乗れないからです。
**△ が実際には5席あっても e5489 からは区別できないため、取りこぼす側に倒しています。**
確実性より機会を優先したい場合は `partySize` を 1 にすると △ でも通知します。
